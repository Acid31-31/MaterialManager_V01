# ╔════════════════════════════════════════════════════════════════════════╗
# ║  MATERIALMANAGER 1.0.x - MULTI-PC INSTALLER CREATOR                     ║
...
Write-Host "║                                                                        ║" -ForegroundColor Cyan
Write-Host "║  🔧 MATERIALMANAGER 1.0.x - MULTI-PC INSTALLER CREATOR                 ║" -ForegroundColor Cyan
...
# ╔════════════════════════════════════════════════════════════════════════╗
# ║  MATERIALMANAGER 1.0.x - INSTALLER SCRIPT                              ║
...
function Write-Header {
    ...
    Write-Host ""
    Write-Host "╔════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
    Write-Host "║  🔧 MATERIALMANAGER 1.0.x INSTALLATION                         ║" -ForegroundColor Cyan
    Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
}
...
function Create-DesktopShortcut {
    ...
    $ShortcutPath = "$DesktopPath\MaterialManager 1.0.x.lnk"
    
    $WshShell = New-Object -ComObject WScript.Shell
    $Shortcut = $WshShell.CreateShortcut($ShortcutPath)
    $Shortcut.TargetPath = $ProgramPath
    $Shortcut.WorkingDirectory = Split-Path $ProgramPath
    $Shortcut.Description = "MaterialManager 1.0.x - Materialverwaltung für Blechlager"
    $Shortcut.Save()
...
function Create-StartMenuShortcut {
    ...
    $ShortcutPath = "$CompanyFolder\MaterialManager 1.0.x.lnk"
...
function Create-UninstallScript {
    ...
    @"
@echo off
REM Deinstallation von MaterialManager 1.0.x
...
echo.
echo Deinstalliere MaterialManager 1.0.x...
echo.
...
del "%USERPROFILE%\Desktop\MaterialManager 1.0.x.lnk" 2>nul
...
Write-Host "   1. Desktop-Shortcut: MaterialManager 1.0.x" -ForegroundColor White
Write-Host "   2. oder Start-Menü: MaterialManager > MaterialManager 1.0.x" -ForegroundColor White
...
$InnoSetupContent = @"
; Inno Setup Script für MaterialManager 1.0.x
; Professioneller Installer für Multi-PC Installation

[Setup]
AppName=MaterialManager 1.0.x
AppVersion=1.0.0
...
DefaultGroupName=MaterialManager 1.0.x
OutputDir=.
...
[Icons]
Name: "{group}\MaterialManager 1.0.x"; Filename: "{app}\MaterialManager_V01.exe"; WorkingDir: "{app}"
Name: "{group}\Anleitung"; Filename: "{app}\Anleitung\QUICK_START.md"
Name: "{group}\Uninstall MaterialManager 1.0.x"; Filename: "{uninstallexe}"
Name: "{commondesktop}\MaterialManager 1.0.x"; Filename: "{app}\MaterialManager_V01.exe"; Tasks: desktopicon
Name: "{commonstartmenu}\MaterialManager 1.0.x"; Filename: "{app}\MaterialManager_V01.exe"; Tasks: startmenuicon

[Run]
Filename: "{app}\MaterialManager_V01.exe"; Description: "MaterialManager 1.0.x starten"; Flags: nowait postinstall skipifsilent
