# ═══════════════════════════════════════════════════════════════════════════════
# 🚀 MATERIALMANAGER 1.0.x → V01 - ADMIN ROOT SCRIPT
#
# DIESES SKRIPT:
# 1. Startet sich selbst mit ADMIN-RECHTEN (falls nicht schon Admin)
# 2. Kopiert 1.0.x → D:\MaterialManager_V01_komplett
# 3. Ändert ALLE 1.0.x → V01 Referenzen
...
Write-Host "║                                                                    ║" -ForegroundColor Green
Write-Host "║     🚀 MATERIALMANAGER 1.0.x → V01 KOMPLETT-MIGRATION 🚀           ║" -ForegroundColor Green
Write-Host "║                                                                    ║" -ForegroundColor Green
...
if (-not (Test-Path $source)) {
    Write-Host "❌ 1.0.x nicht gefunden: $source" -ForegroundColor Red
    pause
    exit 1
}

Write-Host "✅ 1.0.x-Ordner: GEFUNDEN" -ForegroundColor Green
...
Write-Host "[1/5] KOPIERE 1.0.x → D:\MaterialManager_V01_komplett..." -ForegroundColor Yellow
Write-Host ""
...
Write-Host "[2/5] ÄNDERE ALLE 1.0.x → V01..." -ForegroundColor Yellow
Write-Host ""
...
if (Test-Path $oldPath) {
    Write-Host "⚠️  Lösche alte Demo-Daten (1.0.x)..." -ForegroundColor Yellow
    try {
        Remove-Item -Path $oldPath -Recurse -Force -ErrorAction Stop
        Write-Host "✅ Gelöscht" -ForegroundColor Green
    } catch {
        Write-Host "⚠️  Konnte 1.0.x AppData nicht löschen (nicht kritisch)" -ForegroundColor Yellow
    }
}
