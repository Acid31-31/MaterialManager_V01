    ...
    public static class LicenseKeyGenerator
    {
        ...
        public static bool ValidateLicenseKey(string licenseKey, string hardwareId, string registeredTo)
        {
            try
            {
                ...
                var expectedKey = GenerateLicenseKey(hardwareId, registeredTo, 1);
                if (string.Equals(licenseKey, expectedKey, StringComparison.OrdinalIgnoreCase))
                    return true;

                // Versuche vorherigen Tag (falls Generator und Validator an verschiedenen Tagen laufen)
                var yesterday = DateTime.Now.AddDays(-1).AddYears(1).ToString("yyyyMMdd");
                var data = $"{hardwareId}|{registeredTo}|{yesterday}|{MasterSecret}";
                using (var hmac = new HMACSHA256(Encoding.UTF8.GetBytes(MasterSecret)))
                {
                    var hash = hmac.ComputeHash(Encoding.UTF8.GetBytes(data));
                    var hashString = Convert.ToBase64String(hash).Replace("+", "").Replace("/", "").Replace("=", "").Substring(0, 16).ToUpper();
                    var yesterdayKey = $"MM-{hashString.Substring(0, 4)}-{hashString.Substring(4, 4)}-{hashString.Substring(8, 4)}-{hashString.Substring(12, 4)}";
                    if (string.Equals(licenseKey, yesterdayKey, StringComparison.OrdinalIgnoreCase))
                        return true;
                }
